...
$(document).ready(function () {
    ...
    $("#btn_good").click(function () {
        console.log($("#btn_good i").css("color"));
        if ($("#btn_good i").css("color") != "rgb(255, 0, 0)") {
            $("#btn_good i").css("color", "red");
            //——————————————————此处向后端传输点赞信号并更新点赞数
            var new_good_num = parseInt($("#btn_good span").text()) + 1;
            $("#btn_good span").text(new_good_num);
        }
    });
    $("#btn_collect").click(function () {
        var note = document.createElement("div");
        note.className = 'collect_note';
        if ($("#btn_collect i").css("color") != "rgb(255, 0, 0)") { //若没有收藏过，则将图标变色并弹窗提示收藏成功
        ...
        var position_x = document.body.offsetWidth / 2 - 75 + "px";
        var position_y = document.body.offsetHeight / 2 - 25 + "px";
        note.style.left = position_x;
        note.style.top = position_y;
        $("#bar").append(note);
        setTimeout(function () {
            note.remove();
        }, 1000);
...
function ajaxGetHTML(webURL) {
    var url = webURL;
    if (url == "") url = document.getElementById("text").value;
    var xmlhttp;
