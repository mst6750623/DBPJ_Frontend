...
function rank(type) {
  ...
  $.ajax({
    ...
    url: 'api/index' + target,
    data: {
      period: period_length
    },
    success: function(data) {
      for(var i=0;i<3;i++){
        songs[i].imgSrc=data.pic_file[i];
        songs[i].songname=data.music_name[i];
        songs[i].user=data.uploader_name[i];
        songs[i].playNum=data.play_times[i];
        songs[i].likeNum=data.like_time[i];
        songs[i].id=data.music_id[i];
      }
